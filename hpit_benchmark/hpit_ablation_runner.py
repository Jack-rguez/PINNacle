...
def benchmark_pde(pde_name: str, args, device: str):
    ...
    try:
        ...
        if args.checkpoint and os.path.exists(args.checkpoint):
            ...
            # Checkpoint name includes batch size so parallel ablation runs
            # don't collide with the main training runs or each other.
            ckpt_path = RESULTS_DIR / f"hpit_{pde_name}_nophys_bs{args.train_batch_size}.pt"
            ...
            notes_train = f"ablation_no_physics_bs{args.train_batch_size}"
...
def main():
    ...
    parser.add_argument("--lr", type=float, default=1e-3,
                        help="Learning rate (default: 1e-3; use 2e-3 for --train-batch-size 512)")
    parser.add_argument("--train-batch-size", type=int, default=64,
                        help="Training batch size (default: 64; pass 512 for large-batch ablation)")
